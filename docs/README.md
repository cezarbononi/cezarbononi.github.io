...
Personal portfolio site that repositions Cezar Bononi as **Software Engineer | Cloud, Frontend Modernization & AI-assisted Development**.

The chosen direction is **Concept 2 — Cyber Maximalist**, a single-page, dark **controlled chaos** aesthetic with technical grids, microglitch, chrome-like accents, animated overlays and a high-energy visual rhythm.

The goal is to create a portfolio that feels futuristic, expressive and personal, while remaining professional, readable, responsive and production-ready.

The portfolio is fully **bilingual PT-BR / EN-US**, with a header language switcher. The experience should be mobile-first and optimized for both Brazilian and international audiences.

This package documents the **chosen direction (Concept 2)**. The full prototype has 7 sections: Hero, Marquee, About/Manifesto, Selected Work, Stack, Experience and Contact.

## About the Design Files

The files in this bundle (`concept-2-cyber-maximalist.dc.html`, `README.md` and `support.js`) are **design handoff references** generated from Claude Design.

The HTML prototype shows the intended visual direction, motion language, layout structure, typography, colors and interaction behavior. It is **not production code to copy directly**.

The `support.js` file is only the Claude Design prototype runtime. It should **not** be used in the final application build.

Treat the prototype markup as a **visual specification**, not as a component library.

The task is to **recreate this design in the final target stack** using clean, maintainable frontend architecture.
...
The final implementation should reproduce the approved design direction faithfully using the tokens, layout rules, motion specs and content structure documented below, while replacing the prototype runtime with real production-ready React components.

## Fidelity

**High-fidelity (hifi).** Final colors, typography, spacing, motion and interactions are defined here. Recreate the UI faithfully, but adapt the implementation to the final React-based architecture.

The goal is not to copy the exported HTML line by line. The goal is to preserve the approved visual direction, interaction model and content hierarchy in a maintainable production codebase.

## Tech notes / recommended structure

* One single-page portfolio composed of reusable React components:
...
* **i18n**: the prototype stores all copy in a dictionary keyed by `pt` / `en`. Recreate this with a typed content map in TypeScript and a simple language hook/context.

* The language switcher should use:
...
* Persist the selected language in `localStorage` under the key `cb_lang`.

* Default language: `pt`.

* **Scroll reveal**: use Framer Motion or IntersectionObserver to reveal sections with:

  * initial opacity `0`
  * vertical offset around `28px`
  * final opacity `1`
  * final transform `none`
  * transition around `.7s cubic-bezier(.2,.7,.2,1)`

* **Reduced motion**: respect `prefers-reduced-motion` and provide a calmer experience for users who prefer less animation.

* No external image assets are required. Decorative textures, grids, scanlines, glow effects and grain can be recreated with CSS/SVG.

* The Claude Design runtime (`support.js`) must not be imported into the final React application.
